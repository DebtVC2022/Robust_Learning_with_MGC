...
# 定义MCP函数的一阶导数并添加到损失函数的计算中
def mcp_derivative(t, lam, a):
    if 0 <= abs(t) <= a * lam:
        return np.sign(t) * (lam - abs(t) / a)
    elif abs(t) > a * lam:
        return 0
    
def loss_gradient_MCP(y, x, beta, gamma, M, lam=1, a=3.7):
    z = y * (gamma + 1) * np.dot(beta.T, x)
    weight = (np.exp(z) / (1 + np.exp(z))) ** (gamma / (gamma + 1))

    # 添加 MCP 惩罚项的一阶导数
    mcp_grad = np.array([mcp_derivative(b, lam, a) for b in beta])
    
    if weight > M:
        gradient = (gamma / (gamma + 1)) * weight * (sigmoid(z) - 1) * y * x + weight + mcp_grad
    else:
        gradient = (gamma / (gamma + 1)) * weight * (sigmoid(z) - 1) * y * x - weight - mcp_grad

    # 将 MCP 惩罚项的一阶导数与原有梯度相加
    # gradient += mcp_grad
    ...
    for epoch in range(n_epochs):
        ...
        for batch_start in range(0, n_samples, batch_size):
            ...
            for xi, yi in zip(X_batch, y_batch):
                # 不带惩罚项、不带元学习的梯度计算
                gradient += loss_gradient_MCP(yi, xi, beta, gamma, M)
