...
# 定义SCAD函数的一阶导数并添加到损失函数的计算中
def scad_derivative(t, lam, a):
    if 0 <= abs(t) <= lam:
        return np.sign(t) * lam
    elif lam < abs(t) <= a * lam:
        return np.sign(t) * (a * lam - abs(t)) / (a - 1)
    elif abs(t) > a * lam:
        return 0

def loss_gradient_SCAD(y, x, beta, gamma, M, lam=1, a=3.7):
    z = y * (gamma + 1) * np.dot(beta.T, x)
    weight = (np.exp(z) / (1 + np.exp(z))) ** (gamma / (gamma + 1))

    # 添加 SCAD 惩罚项的一阶导数
    scad_grad = np.array([scad_derivative(b, lam, a) for b in beta])
    
    if weight > M:
        gradient = (gamma / (gamma + 1)) * weight * (sigmoid(z) - 1) * y * x + weight + scad_grad
    else:
        gradient = (gamma / (gamma + 1)) * weight * (sigmoid(z) - 1) * y * x - weight - scad_grad

    # 将 SCAD 惩罚项的一阶导数与原有梯度相加
    # gradient += scad_grad
    ...
    for epoch in range(n_epochs):
        ...
        for batch_start in range(0, n_samples, batch_size):
            ...
            for xi, yi in zip(X_batch, y_batch):
                # 不带惩罚项、不带元学习的梯度计算
                gradient += loss_gradient_SCAD(yi, xi, beta, gamma, M)
